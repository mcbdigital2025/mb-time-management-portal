...
const Client = () => {
  ...
  const [reportingClientId, setReportingClientId] = useState(null);
  ...
  const handleGenerateClientReport = async (client) => {
  ...
  const clientColumns = [
    {
      header: "ID",
      render: (c) => (
        <span className="text-xs text-gray-400 font-mono">{c.clientId}</span>
      ),
    },
    {
      header: "Name",
    ...
    { header: "Phone", render: (c) => c.phoneNumber || "—" },
    {
      ...
      render: (c) => (
        <span
          className={`px-2 py-1 rounded-full text-xs font-bold ${c.status === "Active" ? "bg-green-100 text-green-700" : "bg-red-100 text-red-700"}`}
        >
          {c.status}
        </span>
      ),
  ...
  const clientActions = hasEditPermission
    ? [
        {
          label: reportingClientId ? "Sending..." : "Report",
          icon: "report",
          variant: "outline",
          showLabel: true,
          title: "Generate and email client report to reception",
          disabled: (c) => reportingClientId === c.clientId,
          onClick: (c) => handleGenerateClientReport(c),
        },
        {
          label: "Edit",
          icon: "edit",
    ...
    <div className="relative min-h-screen w-full flex flex-col items-center hero-radial-background bg-[radial-gradient(12%_14.08%_at_9.42%_89.81%,#D1E5FF,#F8FAFC),radial-gradient(13.98%_18.61%_at_186.74%_119.73%,rgba(110,178,188,0.4),rgba(217,217,217,0.4))] px-4 py-10">
      <div className="relative z-10 w-full max-w-7xl space-y-6">
        ...
        <div className="bg-white/80 rounded-2xl p-6 border border-teal-100 shadow-sm mt-8">
          ...
          <div
            className={`p-6 rounded-xl border ${clientLearningNeeds ? "bg-white border-teal-100" : "bg-gray-50 border-gray-200 border-dashed"}`}
          >
            {clientLearningNeeds ? (
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-y-6 gap-x-8 text-sm">
                <div>
                  ...
                  <p className="mt-1 font-medium">
                    {clientLearningNeeds.preferredLearningMethod || "N/A"}
                  </p>
                </div>
                <div>
                  <strong className="text-gray-400 uppercase text-[10px] tracking-wider">
                  ...
                  <p className="mt-1 font-medium">
                    {clientLearningNeeds.assistiveTechnologyRequired
                      ? "Yes"
                      : "No"}
                  </p>
                </div>
                <div>
                  ...
                  <p className="mt-1 font-medium">
                    {clientLearningNeeds.literacyLevel || "N/A"}
                  </p>
                </div>
                <div>
                  <strong className="text-gray-400 uppercase text-[10px] tracking-wider">
                  ...
                  <p className="mt-1 font-medium">
                    {clientLearningNeeds.numeracyLevel || "N/A"}
                  </p>
                </div>
                <div>
                  <strong className="text-gray-400 uppercase text-[10px] tracking-wider">
                  ...
                  <p className="mt-1 font-medium">
                    {clientLearningNeeds.digitalSkillsLevel || "N/A"}
                  </p>
                </div>
                <div>
                  <strong className="text-gray-400 uppercase text-[10px] tracking-wider">
                  ...
                  <p className="mt-1 font-medium">
                    {clientLearningNeeds.trainingGoal || "N/A"}
                  </p>
                </div>
              </div>
            ) : (
