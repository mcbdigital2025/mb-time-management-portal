...
const CreateUpdateServiceStaffNotes = ({ user }) => {
    ...
    // --- EFFECT: Initialize ID from Session ---
    useEffect(() => {
        if (!router.isReady) return;

        // Try getting from session storage first, fallback to router query
        ...
        if (finalId) {
            ...
            // If still no ID after router is ready, we have a problem
            setIsPageLoading(false);
            setError("No clientBookingId reference found. Please return to the schedule.");
        }
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if (response.ok) {
                ...
                // Clear session after successful save if desired
                sessionStorage.removeItem('currentClientBookingId');
        ...
        <div className="max-w-4xl mx-auto p-6 bg-white shadow-lg rounded-lg mt-10 border border-gray-200">
            ...
            <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div className="md:col-span-2 flex gap-4 mt-6">
                    ...
                    <button type="button" onClick={() => router.back()} className="flex-1 bg-[#F75D42] text-white py-3 rounded-lg font-bold">
                        Cancel
                    </button>
                </div>
            </form>
