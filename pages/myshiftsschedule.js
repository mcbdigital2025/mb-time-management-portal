...
const MyShiftsSchedule = ({ user }) => {
    ...
    const getSystemTime = () => new Date().toLocaleTimeString('en-GB', {
    ...
    const handleClockIn = async () => {
        ...
        try {
            ...
            if (response.ok) { toast.success(`Clocked in at ${currentTime}`); fetchSchedule(); }
        } catch (error) { toast.error("Clock-in failed."); }
    };
    ...
    const handleClockOut = async () => {
        ...
        try {
            ...
            if (response.ok) { toast.success(`Clocked out at ${currentTime}`); fetchSchedule(); }
        } catch (error) { toast.error("Clock-out failed."); }
    };
    ...
    const menuBtnClass = (isActive) => `flex items-center gap-3 px-4 py-3 text-sm font-semibold rounded-xl transition w-full ${
        ...
        <div className="flex min-h-screen bg-gray-50">
            <div className="w-64 bg-white border-r border-slate-200 p-4 flex flex-col gap-2 shadow-sm">
                <div className="mb-6 px-2">
                    <h2 className="text-xs font-bold text-slate-400 uppercase tracking-widest">Shift Actions</h2>
                    <p className="text-[10px] text-slate-500 mt-1">
                        {selectedShift ? `Selected: ${selectedShift.facilitiesName}` : 'Select a shift to enable menus'}
                    </p>
                </div>

                <button disabled={!selectedShift} onClick={handleClockIn} className={`${menuBtnClass(!!selectedShift)} hover:text-emerald-700 hover:bg-emerald-50`}><LogIn size={18} /> Clock In</button>
                ...
                <div className="max-w-4xl mx-auto">
                    <div className="flex flex-col md:flex-row md:items-center justify-between bg-white p-4 rounded-2xl shadow-sm border border-slate-200 mb-6 gap-4">
                        ...
                        <div className="flex items-center gap-2">
                            <button onClick={goToday} className="px-3 py-1.5 text-xs font-semibold bg-slate-50 border border-slate-200 rounded-lg hover:bg-slate-100 transition">Today</button>
                        ...
                        {filteredSchedules.sort((a, b) => new Date(a.workDate) - new Date(b.workDate)).map((shift) => (
                            ...
                            >
                                <div className="flex-1">
                                    ...
                                    <h3 className="text-lg font-bold text-slate-900 mt-1">{shift.facilitiesName}</h3>
                                    <p className="text-sm text-slate-500 mt-1 flex items-center gap-1">
                                        <Clock size={14} className="text-slate-400" /> {shift.scheduledStartTime} - {shift.scheduledEndTime}
                                    </p>
                                </div>
                                <div className="flex flex-col items-end gap-2">
                                    ...
                                    <div className="flex flex-col text-[10px] font-bold text-right space-y-0.5">
                                        <div className="flex items-center justify-end gap-1 text-slate-500">
                                            <MapPin size={12} className="text-slate-400" /> {shift.workLocation || "No Location"}
                                        </div>
                                       <div className="text-emerald-600 font-black">Clock In: {shift.actualStartTime}</div>
                                       <div className="text-rose-600 font-black">Clock Out: {shift.actualEndTime}</div>
                                    </div>
                                </div>
                            </div>
                        ))}
